...
export default function AdminDashboardPage() {
  ...
  const newLeads = stats?.newLeads ?? null;
  const newLeadsUrgent = Boolean(newLeads && newLeads > 0);
        ...
        <>
          ...
          <div className="mx-auto max-w-3xl px-5 py-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              ...
              <Link
                href="/admin/leads"
                className="bg-white rounded-2xl shadow p-5 flex flex-col items-center gap-2 border-b-4 border-b-purple-500 hover:shadow-lg transition text-center"
              >
                <Inbox className="h-9 w-9 text-navy-900" aria-hidden="true" />
                <span className="font-bold text-navy-900 text-lg">Leads</span>
                <span
                  className={`text-sm font-bold px-3 py-1 rounded-full ${
                    newLeadsUrgent ? "bg-purple-500 text-white" : "bg-slate-100 text-slate-500"
                  }`}
                >
                  {stats ? countLabel(newLeads) : "…"}
                </span>
              </Link>
              ...
              <Link
                href="/admin/services"
                className="bg-white rounded-2xl shadow p-5 flex flex-col items-center gap-2 border-b-4 border-b-teal-500 hover:shadow-lg transition text-center"
              >
                <Wrench className="h-9 w-9 text-navy-900" aria-hidden="true" />
                <span className="font-bold text-navy-900 text-lg">Our Services</span>
              </Link>
            </div>
